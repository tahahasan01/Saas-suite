...
export function LeadDrawer({ leadId, onClose, onChanged }: { leadId: string; onClose: () => void; onChanged: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-20 flex justify-end bg-black/50" onClick={onClose}>
      <div className="h-full w-full max-w-md overflow-y-auto border-l border-gray-800 bg-[#0e1420] p-6" onClick={(e) => e.stopPropagation()}>
        {!data ? (
          ...
          <>
            <div className="mb-4 flex items-start justify-between">
              <div>
                ...
                {data.lead.company && <p className="text-sm text-gray-400">{data.lead.company}</p>}
              </div>
            ...
            <form onSubmit={submitLog} className="mb-6 space-y-2 rounded-lg border border-gray-800 p-3">
              <p className="text-xs font-semibold text-gray-300">Log interaction</p>
              <div className="grid grid-cols-2 gap-2">
                <select className="rounded-md border border-gray-700 bg-gray-900 px-2 py-1.5 text-sm"
              ...
              <textarea placeholder="Call notes…" value={log.note} onChange={(e) => setLog({ ...log, note: e.target.value })}
                        className="w-full rounded-md border border-gray-700 bg-gray-900 px-2 py-1.5 text-sm" rows={2} />
              <label className="block text-xs text-gray-500">
                Next follow-up
                <input type="datetime-local" value={log.follow} onChange={(e) => setLog({ ...log, follow: e.target.value })}
                       className="mt-1 w-full rounded-md border border-gray-700 bg-gray-900 px-2 py-1.5 text-sm" />
              </label>
              <Button type="submit" disabled={busy} className="w-full">{busy ? "Saving…" : "Log"}</Button>
            </form>

            <p className="mb-2 text-xs font-semibold text-gray-400">Timeline</p>
            <ul className="space-y-3">
              {data.interactions.length === 0 && <li className="text-sm text-gray-600">No interactions yet.</li>}
              {data.interactions.map((i) => (
                <li key={i.id} className="border-l-2 border-gray-700 pl-3 text-sm">
                  <p className="text-gray-300">
                    <span className="font-medium capitalize">{i.channel}</span>
                    {i.outcome && <span className="text-indigo-400"> · {i.outcome.replace("_", " ")}</span>}
                  </p>
                  {i.note && <p className="text-gray-400">{i.note}</p>}
